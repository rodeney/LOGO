 ...
 */
case class RspFileSourceScanExec(
  ...
  private def mycreateNonBucketdReadRDD(
                                         readFile: (PartitionedFile) => Iterator[InternalRow],
                                         selectedPartitions: Seq[PartitionDirectory],
                                         fsRelation: HadoopFsRelation): RspRDD[InternalRow] = {
    val splitFiles = selectedPartitions.flatMap { partition =>
      partition.files.flatMap { file =>
        val blockLocations = getBlockLocations(file)
        val hosts = getBlockHosts(blockLocations, 0, file.getLen)
    ...
    new RspRDD(new FileScanRDD(fsRelation.sparkSession, readFile, partitions))
  }
  ...
  override def doCanonicalize(): RspFileSourceScanExec = {
    RspFileSourceScanExec(
      relation,
